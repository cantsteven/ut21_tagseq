...
m_res_df$symbol <- mapIds(org.Rn.eg.db, 
                          keys = rownames(m_res_df), 
                          keytype = "ENSEMBL", 
                          multiVals = "first",
                          column = "SYMBOL")
...
f_res_df$symbol <- mapIds(org.Rn.eg.db, 
                          keys = rownames(f_res_df), 
                          keytype = "ENSEMBL",
                          multiVals = "first",
                          column = "SYMBOL")
...
mf_volcano <- EnhancedVolcano(mf_res_df,
                             ...
                             col = c("#D3D3D3", "gray50", "#8B8589", "#8B8589")
                             ) + 
  ggtitle("Female vs Male Oil") + 
  annotate("text", x = 1, y = 22, label = "↑ in females", hjust = 0, size = 5, color = 'black') +
  annotate("text", x = -3.5, y = 22, label = "↑ in males", hjust = 0, size = 5, color = 'black') +
...
mf_genelist <- mf_res_df$log2FoldChange
names(mf_genelist) <- mf_res_df$symbol
mf_genelist = sort(mf_genelist, decreasing = TRUE)

gse_mf <- gseGO(geneList=mf_genelist, 
             ont ="BP", 
             keyType = "SYMBOL", 
             minGSSize = 3, 
             maxGSSize = 800, 
...
gse_mf_graph <- ggplot(gse_mf_df, aes(NES, fct_reorder(Description, NES), fill = NES > 0)) + 
    geom_col(orientation='y') + 
    theme_prism(base_size = 9) + 
    scale_fill_manual(values = c("TRUE" = "#8B8589", "FALSE" = "#8B8589")) +
...
# Figure 2 Creation---
free(volcano_m + volcano_f) / free(gse_m_graph + gse_f_graph) / (m_kegg_graph + f_kegg_graph) + plot_layout(height = c(2,2,2)) + plot_annotation(tag_levels = 'A') 
...
volcano_m <- EnhancedVolcano(m_res_df,
                             ...
                             gridlines.major = FALSE,
                             gridlines.minor = FALSE,
                             col = c("#D3D3D3", "#96A9C5", "#73B2FA", "#73B2FA")
                             ) + 
  ggtitle("Males") + 
  annotate("text", x = 1.5, y = 8.7, label = "↑ with flutamide", hjust = 0, size = 5, color = 'black') +
  annotate("text", x = -3.5, y = 8.7, label = "↓ with flutamide", hjust = 0, size = 5, color = 'black') +
...
volcano_f <- EnhancedVolcano(f_res_df,
                             ...
                             gridlines.major = FALSE,
                             gridlines.minor = FALSE,
                             col = c("#D3D3D3", "#C29A6D", "#F19837", "#F19837")
                             ) + 
  ggtitle("Females") + 
  annotate("text", x = 1, y = 8.7, label = "↑ with testosterone", hjust = 0, size = 5, color = 'black') +
  annotate("text", x = -3.5, y = 8.7, label = "↓ with testosterone", hjust = 0, size = 5, color = 'black') +
...
# Figure 2 Creation---
free(mf_volcano + gse_mf_graph) / free(volcano_m + volcano_f) / (gse_m_graph + gse_f_graph) + plot_layout(height = c(2,2,2)) + plot_annotation(tag_levels = 'A') 

ggsave("/stor/scratch/Fonken/archive/UT21_tagseq/ut21_tagseq/new_figurevolcanogokegg.svg", dpi = 300, width = 12, height = 15)
```

## Looking at individual genes 

```{r}
#extracting the counts for genes of interest (use whatever deseq object you want)
mf_genelist<- counts(mf_dds, normalized = FALSE)%>% 
  as.data.frame()%>%
  ...
  filter(symbol %in% c("Clu","Gfap", "Apln", "Lxn", "Grin2c", "Aqp4", "Pltp", "Htra1", "Esr1", "Esr2")) #here is where you can input your genes of interest, I looked at a bunch of astrocyte genes 
...
gfapgene<- genelist%>%
  ...
  {colnames(.)[2] <- "count"; .}%>%
  full_join(group_info_oil, by = "id") #(you need to join some group info to this data frame)
